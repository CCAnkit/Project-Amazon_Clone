...
const updateProfile = async (req, res) => {
    try{
        ...
        const { fname, lname, email, phone, password } = data           //Updates a User by changing these values 
        ...
        if(address){
            if(Object.keys(address).length > 0){
                if(!(address.shipping)){
                    if(!validator.isValidValue(address.shipping)){
                        return res.status(400).send({status: false, message: "Please provide the Shipping Address"})   //Shipping is mandory 
                    }
